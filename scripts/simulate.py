...
def scenario_high_amount(user_id):
    return {
        ...
        "upi_lat": 28.6139 + random.uniform(-0.01, 0.01),
        "upi_lon": 77.2090 + random.uniform(-0.01, 0.01),
...
def scenario_new_location(user_id):
    return {
        ...
        "location": "Mumbai",
        "upi_app": random.choice(["GPay", "PhonePe", "Paytm"]),
        "upi_lat": 19.0760 + random.uniform(-0.02, 0.02),
        "upi_lon": 72.8777 + random.uniform(-0.02, 0.02),
        "device_id": "device_" + str(random.randint(1, 100)),
        "merchant": "Grocery",
        "timestamp": f"{random.randint(0,23)}:{random.randint(0,59):02d}",
    }
